...
# CSS for blue background, thin borders and fixed header
page_css = """
<style>
    .stApp {
    ...
    .table-header, .table-row {
        display: grid;
        grid-template-columns: 50px 3fr 2fr 2fr 3fr 70px 70px;
        align-items: center;
...
def open_topics():
    ...
    with container:
        # We use empty placeholders for buttons to capture clicks
        for idx, row in df_open.iterrows():
            row_id = row["row_id"]
            cols = st.columns([0.07, 0.6, 0.4, 0.4, 0.6, 0.12, 0.12])
            cols[0].write(idx + 1)
